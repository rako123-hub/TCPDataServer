...
void CServerSock::init()
{
	...
	*/
    m_acceptThread = std::thread(&CServerSock::doAcceptStub, this);
    m_connectThread = std::thread(&CServerSock::doCreateConnect, this);
...
//			RETAILMSG(1, (TEXT("Loop: rd: %i, cnt: %i, Sock: %i\r\n"),	m_Read,m_Count,m_Sock[m_Read]));
			...
			{
				CConnectSock *pSock = new CConnectSock(socket, pDataProvider);
				pSock->destroy();
				pSock = nullptr;
